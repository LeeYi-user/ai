...
def checkWinner(board, player):
    ...
    for i in range(size):
        for j in range(size - 4):
            # 檢查水平方向
            if (board[i][j:j+5] == [player]*5).all():
                return True
            # 檢查垂直方向
            if all(board[x][i] == player for x in range(j, j+5)):
                return True
    ...
    for i in range(size - 4):
        for j in range(size - 4):
            # 左上到右下對角線
            if all(board[i+x][j+x] == player for x in range(5)):
                return True
            # 右上到左下對角線
            if all(board[i+4-x][j+x] == player for x in range(5)):
                return True

    return False

def printBoard(humanFirst, board):
    size = len(board)
    
    # Print column labels (A-Z)
    print("   ", end="")
    for i in range(size):
        print(chr(65 + i), end=" ")
    print()
    
    # Print rows with row labels and board contents
    for i in range(size):
        print(f"{i+1:2d} ", end="")
        for j in range(size):
            if board[i][j] == 0:
